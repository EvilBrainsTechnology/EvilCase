...
public class PartyModelTests
{
    ...
    {
        var party = ModelFixture.Model.FindEntityType(typeof(Party));
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ModelFixture.IsIndexed(party, nameof(Party.OwnerId)), Is.True, "every owner-scoped query reads this index");
        }
    }
    ...
    public void APartyIsFlatAndItsAddressIsOneBlock()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ModelFixture.IsIndexed(party, nameof(Party.DataBoxId)), Is.True, "looking a party up by data box is the one unambiguous lookup");
        }
    }
}
