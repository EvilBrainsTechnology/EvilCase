...
- One `IQueryable` extension step per rule, composed by a reader; a step returns `IQueryable`,
  and the last one projects into the contract DTO.
- The reader composes the steps and calls `ToListAsync` once, at the end.
...
on `TenantId` and leads its unique indexes with it. The user carries the filter, not the index: its
e-mail is unique deployment-wide, and a lookup before a tenant is known ignores the filter.
`SaveChanges` refuses another tenant's row, fills a user-owned row's `UserId` from it and refuses
another user's row. Work outside a request enters both ids at once.
