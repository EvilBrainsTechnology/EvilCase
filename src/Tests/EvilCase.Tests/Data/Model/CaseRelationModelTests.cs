...
public class CaseRelationModelTests : ModelFixture
{
    [Test]
    public void ARelationIsOneRowPerOrderedPairOfDistinctCases()
    {
        var designTime = DesignTimeModel.FindEntityType(typeof(CaseRelation));
        var relation = Model.FindEntityType(typeof(CaseRelation));
    ...
    public void DeletingACaseTakesItsRelationsAndLeavesTheCasesItRelatedTo()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(
                Model.GetEntityTypes().Any(entityType => entityType.ClrType == typeof(Case)
