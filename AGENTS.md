...
- App shell: `Layout/MainLayout.razor` (Tabler `page` + `page-wrapper`) and `Layout/NavMenu.razor`, modelled on Nginx Proxy Manager's horizontal Tabler layout — a top bar with brand and theme switch, and a menu row below it from `lg` up. Below `lg` the hamburger opens the navigation as an offcanvas via `IOffcanvasService`. Both render the same `Layout/NavLinks.razor`.
