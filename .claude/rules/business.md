...
on `TenantId` and leads its unique indexes with it; `SaveChanges` refuses another tenant's row and
fills a user-owned row's `UserId` from it. Work outside a request enters both ids at once.
