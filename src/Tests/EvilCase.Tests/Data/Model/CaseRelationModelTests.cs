...
public class CaseRelationModelTests
{
    ...
    {
        var designTime = DesignTime.FindEntityType(typeof(CaseRelation));
        var relation = Runtime.FindEntityType(typeof(CaseRelation));
    ...
    public void DeletingACaseTakesItsRelationsAndLeavesTheCasesItRelatedTo()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(
                Runtime.GetEntityTypes().Any(entityType => entityType.ClrType == typeof(Case)
