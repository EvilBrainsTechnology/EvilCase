...
- **A session that starts while the loop is meant to be running arms it.** `list_triggers` first; if no Routine exists for the loop, create one. The four Routine tools — `list_triggers`, `create_trigger`, `update_trigger`, `delete_trigger` — need a permission the agent must not grant itself; `.claude/settings.json` allows them, and a denial there is a question for the owner rather than something to work around.
- **Repair the Routine in place, and delete one only when the owner ends the loop.** A Routine whose cadence or prompt is wrong is fixed with `update_trigger`; `delete_trigger` is for a duplicate Routine, or for the loop the owner has stopped. Removing the loop's only clock on the agent's own initiative is the silent stop this section exists to prevent.
