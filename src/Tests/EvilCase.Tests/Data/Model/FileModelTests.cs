...
public class FileModelTests
{
    ...
    {
        var asset = ModelFixture.Model.FindEntityType(typeof(FileAsset));
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ModelFixture.IsIndexed(asset, nameof(FileAsset.ActId)), Is.True, "an act reads its own files through this index");
        }
    }
    ...
    {
        IReadOnlyEntityType?[] mapped = [ModelFixture.Model.FindEntityType(typeof(FileAsset)), ModelFixture.Model.FindEntityType(typeof(ActFileReference))];
    ...
    {
        var reference = ModelFixture.Model.FindEntityType(typeof(ActFileReference));
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ModelFixture.IsIndexed(reference, nameof(ActFileReference.ActId)), Is.True, "an act reads the files it borrows through this index");
            Assert.That(ModelFixture.IsIndexed(reference, nameof(ActFileReference.FileAssetId)), Is.True, "and which acts reference one asset is a lookup, never a table scan");
        }
    }
    ...
    public void AnActThatOwnsAFileAnotherActReferencesCannotBeDeleted()
    {
        ...
        var assetToAct = ModelFixture.ForeignKeyTo<Act>(asset!);
        var referenceToAct = ModelFixture.ForeignKeyTo<Act>(reference!);
        var referenceToAsset = ModelFixture.ForeignKeyTo<FileAsset>(reference!);
    ...
    public void NothingAboutAFileIsARole()
    {
        ...
        var columns = fileTypes.SelectMany(type => ModelFixture.ColumnsOf(ModelFixture.Model.FindEntityType(type))).ToList();
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ModelFixture.Naming(columns, "Role"), Is.Empty, "a role column is back on a file");
            Assert.That(ModelFixture.Naming(kernel, "FileRole"), Is.Empty, "a file role enum is back in the shared kernel");
        }
    }
}
