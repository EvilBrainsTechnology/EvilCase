...
dependency-free shared kernel, `Common/EvilCase.Auth` authentication, `Common/EvilCase.Files` file
storage on disk. `Data/EvilCase.Data` is the EF Core model (PostgreSQL) and the way to the
database; `Data.Migrations` its migrations. `Tests/EvilCase.Tests` holds the application tests
(NUnit), `Utils/EvilBrains.*` the shared libraries and their tests.
