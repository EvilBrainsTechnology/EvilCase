...
- Docker, for `dotnet r test`. The tests start their own PostgreSQL through Testcontainers, as the container evilcase-test-db, and take it down when the run ends. `EVILCASE_TEST_POSTGRES` points them at a server of your own instead, as `Host=…;Port=…;Username=…;Password=…` without a database.
