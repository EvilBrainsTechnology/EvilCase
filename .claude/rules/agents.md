...
- A delegated implementation is followed by a code review from a fresh subagent; the session
  that ordered it works the relevant findings in before it reports or takes up anything else.
  An approval arriving while that review runs changes nothing — the merge waits for it.
