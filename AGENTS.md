...
- **A round runs in a subagent, and a subagent starts blank.** The main thread keeps the session — the pull request subscriptions and the conversation the owner reads — and delegates the work, so an hour of build logs never reaches the context that has to last. The consequence is the rule: **the repository and GitHub are the only memory the loop has.** A finding becomes an issue, the state of a pull request its diff does not show becomes a comment on it, a rule becomes a line here. Something only the current chat knows is something the next round will not know, and the loop will not notice it forgot.
