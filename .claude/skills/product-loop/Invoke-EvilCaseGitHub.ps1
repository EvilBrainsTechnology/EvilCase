...
<#
    ...
    stdout is the response, verbatim. -Select prints named properties instead — dotted paths for
    nested ones, tab-separated, one line per element of an array — so `-Select number` after opening
    a pull request is its number and nothing else, with no second call. A GET is followed through
    its `Link: rel="next"` pages; only then is the response reformatted, as the joined array.
...
foreach ($item in $data) {
    Write-Output (($Select | ForEach-Object { Get-Field $item $_ }) -join "`t")
}
