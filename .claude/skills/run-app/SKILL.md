...
Toolchain, certificate and PostgreSQL per `README.md`; `-PostgresHost` and the parameters
beside it reach another server. `src/EvilCase.Host/.env` needs a JWT key of at least
32 characters and `EvilBrains__EvilCase__Auth__Seed__Email` and `__Password` — the seeded
administrator is the only way into an empty database; the script supplies neither. The file
storage root comes from `appsettings.Development.json`, not `.env`.
...
nothing but the URL, on stdout. `-Stop` kills the run's host, drops its database and removes
its files; the script's header documents the parameters.
...
- `GET /api/cases` with the bearer → `{"items":[]}` until cases exist, without it `401`.
