...
{
    public static string DayPrefix(string caseNumber, DateOnly date) => $"{caseNumber}/{NumberTail.DayPrefix(date)}";

    public static string Compose(string caseNumber, DateOnly date, int sequence) =>
        $"{caseNumber}/{NumberTail.Compose(date, sequence)}";

    /// <summary>Throws <see cref="FormatException"/> where the value is not an act number.</summary>
    public static ActNumberParts Parse(string value) =>
        ParseOrDefault(value) ?? throw new FormatException($"'{value}' is not an act number.");

    /// <summary>Null where the value is not an act number.</summary>
    ...
    /// <summary>The day's next free sequence under this case number. A value outside the format does not count.</summary>
