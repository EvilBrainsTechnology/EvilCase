...
- HTTP calls are logged and shipped like anything else, so the server sees the browser's request trail. The one exception is the upload itself: a global filter drops events of the upload client (a typed client logs under its interface name) that carry no exception. Without it every upload would log four more events and the next upload would ship them, forever. A failed upload is kept — it settles on its own, because the batch that could not be sent is dropped rather than retried.
