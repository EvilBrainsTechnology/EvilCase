...
`EvilCase.Data` holds the schema and the database session (SDD-018). Read the model SDDs under
`docs/sdd/` and the fixtures under `Tests/Data/Model/` before changing an entity; below is what
neither says.
...
- The application reads and writes through `IApplicationDbSession`; a `DbContext` never leaves
  its DI scope.
