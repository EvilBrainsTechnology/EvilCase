...
- **An AI agent never merges a pull request and never pushes to `master`.** Not its own, not anyone else's, not after a green CI run, not when asked to "finish" or "ship" something — merging is the owner's, always, and there is no case that overrides this. That covers `gh pr merge`, auto-merge, a direct push to `master` and any branch protection change that would allow one. An agent opens the pull request and stops there.
