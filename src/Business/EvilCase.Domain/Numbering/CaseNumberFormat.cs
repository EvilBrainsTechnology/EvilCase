...
public static class CaseNumberFormat
{
    ...
    public static string DayPrefix(DateOnly date) => Prefix + NumberTail.DayPrefix(date);

    public static string Compose(DateOnly date, int sequence) => Prefix + NumberTail.Compose(date, sequence);

    /// <summary>Throws <see cref="FormatException"/> where the value is not a case number.</summary>
    ...
    /// <summary>Null where the value is not a case number.</summary>
    ...
    /// <summary>The day's next free sequence. A value outside the format, or of another day, does not count.</summary>
