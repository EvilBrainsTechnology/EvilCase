...
public class FileModelTests : ModelFixture
{
    ...
    {
        IReadOnlyEntityType?[] mapped = [Model.FindEntityType(typeof(FileAsset)), Model.FindEntityType(typeof(ActFileReference))];
    ...
    {
        var reference = Model.FindEntityType(typeof(ActFileReference));
    ...
    public void NothingAboutAFileIsARole()
    {
        ...
        var columns = fileTypes.SelectMany(type => ColumnsOf(Model.FindEntityType(type))).ToList();
